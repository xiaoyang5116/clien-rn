...
const PropSelector = (props) => {
    ...
    React.useEffect(() => {
        AppDispath({ type: 'PropsModel/getPropsFromAttr', payload: { attr: props.attrFilter }, cb: (data) => {
            if (data.length > 0) {
        ...
        const listener = DeviceEventEmitter.addListener('__@PropSelector.select', (e) => {
    ...
    const renderItem = (data) => {
        ...
        <TouchableOpacity activeOpacity={1} onPress={() => {
            ...
            setTimeout(() => {
                DeviceEventEmitter.emit('__@PropSelector.select', { tag: props.tag, e: data.item });
        ...
        <View style={selectorStyles.selectorContainer}>
            <View style={selectorStyles.selectorView}>
                ...
                <View style={selectorStyles.headerView}>
                    <Text style={{ fontSize: 22, color: '#000' }}>选择道具</Text>
                </View>
                {
                (props.currentPropId > 0)
                ? (
                <View style={selectorStyles.rmView}>
                    <View style={{ width: 80, marginLeft: 12 }}>
                        <TextButton title={'卸下'} onPress={() => {
                            DeviceEventEmitter.emit('__@PropSelector.select', { tag: props.tag, e: null });
                            if (props.onClose != undefined) {
                                props.onClose();
                            }
                        }} />
...
PropSelector.propTypes = {
    attrFilter: PropTypes.string,
    tag: PropTypes.string,
    currentPropId: PropTypes.number,
    onClose: PropTypes.func,
    onSelected: PropTypes.func,
}

PropSelector.defaultProps= {
}

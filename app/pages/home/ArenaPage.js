...
const ArenaPage = (props) => {
    ...
    if (props.report.length > 0 && reportIndex.current < props.report.length) {
        ...
        if (props.myself.uid == item.attackerUid) {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: '#403340' }}>
            <View style={props.currentStyles.viewContainer}>
                ...
                <Character user={props.enemy} contentStyle={{ marginTop: px2pd(80) }} />
                    ...
                    <FlatList
                        ...
                        onContentSizeChange={() => {
                            if (listData.current.length > 0) {
                                refList.current.scrollToIndex({ index: listData.current.length - 1 });
                            }
                        }}
                    />
                </View>
                <Character user={props.myself} />
            </View>
        </SafeAreaView>
    );
}
